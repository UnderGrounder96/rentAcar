...
router.get('/:vehicle_id', (req, res) => {
  db.query(`SELECT * FROM cars
    WHERE idCar LIKE ?
    AND active NOT LIKE 0;`, req.params.vehicle_id, (err, result) => {
    if (!result.length)
      ...
      db.query(`SELECT idCar, photo, model FROM cars
        ...
        ORDER BY RAND()
        LIMIT 10;`, req.params.vehicle_id, (err1, result1) => {
        if (req.user) {
          db.query(`SELECT * FROM reservations
            WHERE idCar LIKE ?
            AND idUser LIKE ?
            AND DATE(dateOut)>=CURDATE()
            AND active NOT LIKE 0;`, [req.params.vehicle_id,
              req.user.idUser], (err2, result2) => {
...
router.post('/edit/:res_id/', (req, res) => {
  ...
  db.query(`UPDATE reservations
    SET fullPrice=?,
      dateIn=?,
      dateOut=?
    WHERE idReservation LIKE ?;`, [req.body.numbers,
